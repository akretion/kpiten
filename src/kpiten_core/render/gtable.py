...
def _drawing(table: GT, df: pl.DataFrame, palette: dict, drawing: dict, rows: int):
    """The heatmap, the « Total » row set apart, the note and the options of `[table]`."""
    ...
    if drawing.get("totals"):  # the background of the header, a rule on the rows side
        total = 0 if first else rows
        table = table.tab_style(
            style=[
                style.text(weight="bold"),
                style.fill(color=palette["thead"]),
                style.borders(
                    sides="bottom" if first else "top",
                    color=palette["border_hex"],
                    weight="2px",
                ),
            ],
            locations=[loc.body(rows=[total]), loc.stub(rows=[total])],
        )
    if drawing.get("note"):
