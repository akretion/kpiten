...
Standard library only (`tomli` before Python 3.11) : Odoo imports this package on
every series it runs on (Python 3.8 and after), without the rest of kpiten-core.
"""
...
# the first word of a SQL definition, after its comments (a polars one : `d_next = d`)
...
def display(definition: str, field: str | None = None) -> dict:
    ...
    header = sql_header(definition) if is_sql(definition) else ""
